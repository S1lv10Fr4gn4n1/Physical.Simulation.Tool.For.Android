...
{
	if (Controller::controller) {
		Controller::controller->stopSimulation();
...
void Controller::generateRandonSimulation(unsigned int _numberObjects)
{
    this->mainEngine->deleteAllSimulatedObjects();
    ...
    for (int i=0; i< _numberObjects; i++) {
        ...
        // torque
        x = 0.0f;//getRand(-0.5f, 0.5f);
        y = 0.0f;//getRand(-0.5f, 0.5f);
        z = 0.0f;//getRand(-0.5f, 0.5f);
